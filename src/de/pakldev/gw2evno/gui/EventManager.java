	...
	private void checkForNewStates() {
				...
				if( lastEventState.containsKey(eventId) ) {
					...
					if( oldState != newState ) {
						...
						if( newState == Events.STATE_WARMUP ) {
							dm.newDialog("<html><center><b>" + eventName + "</b><br />" + Language.warmup() + "</center></html>", EventNames.getIcon(eventId), EventNames.getWikiURL(eventId), false);
						} else if( newState == Events.STATE_PREPARATION ) {
							dm.newDialog("<html><center><b>" + eventName + "</b><br />" + Language.preparation() + "</center></html>", EventNames.getIcon(eventId), EventNames.getWikiURL(eventId), false);
						} else if( newState == Events.STATE_ACTIVE ) {
							dm.newDialog("<html><center><b>" + eventName + "</b><br />" + Language.active() + "</center></html>", EventNames.getIcon(eventId), EventNames.getWikiURL(eventId), false);
						}
					}
				}
			}
		...
		for(String eventId : InterestingEvents.eventIds) {
			...
			if( oldState != newState ) {
				...
				if( newState == Events.STATE_WARMUP ) {
					dm.newDialog("<html><center><font color=\"#FFFFFF\"><b>" + eventName + "</b><br />" + Language.warmup() + "</font></center></html>", EventNames.getIcon(eventId), EventNames.getWikiURL(eventId), true);
				} else if( newState == Events.STATE_PREPARATION ) {
					dm.newDialog("<html><center><font color=\"#FFFFFF\"><b>" + eventName + "</b><br />" + Language.preparation() + "</font></center></html>", EventNames.getIcon(eventId), EventNames.getWikiURL(eventId), true);
				} else if( newState == Events.STATE_ACTIVE ) {
					dm.newDialog("<html><center><font color=\"#FFFFFF\"><b>" + eventName + "</b><br />" + Language.active() + "</font></center></html>", EventNames.getIcon(eventId), EventNames.getWikiURL(eventId), true);
				}
